...
# Step 1: Get household data using correct variables
cat("=== STEP 1: HOUSEHOLD DATA ANALYSIS ===\n")

# Get 2020 ACS data - B11010 has nonfamily households by age
...
acs_2020 <- get_acs(
  ...
  variables = c(
    ...
    "B11010_001",   # Total nonfamily households
...
mismatch_analysis <- household_changes %>%
  ...
  mutate(
    # Create mismatch index: solo 65+ growth * large housing percentage
    mismatch_index = solo_65plus_pct_change * large_units_pct / 100,
    
    # Alternative: current solo 65+ share vs small unit share
...
p2 <- mismatch_analysis %>%
  ...
  labs(
    ...
    subtitle = "Each point is a California county • Size = population",
...
cat("\nHYPOTHESIS EVALUATION:\n")
cat("Solo 65+ households are the fastest-growing household type:\n")
cat("  - Overall growth 2010-2020: ", round(overall_growth_rate, 1), "%\n")
cat("  - Mean county-level increase: ", round(mean(mismatch_analysis$solo_65plus_pct_change), 2), " pp\n")

if (cor_test1$p.value < 0.05) {
  cat("\nCorrelation with housing stock:\n")
  cat("  - ", ifelse(cor_test1$estimate < 0, "NEGATIVE", "POSITIVE"), 
      " correlation (r = ", round(cor_test1$estimate, 3), ")\n")
  if (cor_test1$estimate < 0) {
    cat("  - Solo seniors tend to be in areas with SMALLER homes\n")
    cat("  - This may indicate they've already downsized or live in urban areas\n")
  } else {
    cat("  - Solo seniors tend to be in areas with LARGER homes\n")
    cat("  - This confirms the housing mismatch hypothesis\n")
  }
} else {
  cat("\nNo significant correlation with housing stock size found\n")
}

cat("\nHOUSING MISMATCH CONFIRMED: Many counties show high solo senior growth\n")
cat("combined with housing stock dominated by large family homes.\n")
