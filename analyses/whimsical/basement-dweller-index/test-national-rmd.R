# Test the updated .Rmd with national data
...
cat("Testing national .Rmd chunks...\n")
...
# Data analysis chunk
cat("Running national data analysis chunk...\n")
...
# Retrieve REAL data from Census API with geometry - NATIONAL
start_time <- Sys.time()
real_data <- get_acs(
  geography = "county",
...
cat("✓ Retrieved", nrow(real_data), "counties in", round(as.numeric(end_time - start_time), 1), "seconds\n")
...
# Test Executive Summary inline calculations
...
# Test top/bottom county listings
top_counties <- head(arrange(sample_data, desc(basement_dweller_pct))$NAME, 3)
bottom_counties <- head(arrange(sample_data, basement_dweller_pct))$NAME, 3)

cat("Highest rates counties:", paste(top_counties, collapse = ", "), "\n")
cat("Lowest rates counties:", paste(bottom_counties, collapse = ", "), "\n")

cat("\n✓ All national .Rmd chunks work correctly!\n")